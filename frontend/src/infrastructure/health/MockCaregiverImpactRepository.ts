import rawData from '../../data/caregiver_impact_raw.json'
import type {
  AgeGroupCode,
  CaregiverKpiSummary,
...
function mapToRecord(item: RawItem, index: number): CaregiverProfessionalImpactRecord {
  ...
  // Asignación determinista de provincia y coordenadas sintéticas
...
const ALL_RECORDS: CaregiverProfessionalImpactRecord[] = (rawData as RawItem[]).map((item, index) =>
  mapToRecord(item, index),
)

export function getAllCaregiverImpactRecords(): Promise<CaregiverProfessionalImpactRecord[]> {
  return Promise.resolve(ALL_RECORDS)
}

export async function getCaregiverKpiSummary(): Promise<CaregiverKpiSummary> {
...
export async function getAgeGroupImpactSeries(): Promise<AgeGroupImpactPoint[]> {
  const records = ALL_RECORDS.filter(
    (r) => r.sexCode === 'total' && r.professionalProblemsCode === 'sitieneproblemasprofesionales' && r.ageGroupCode !== 'total',
  )
...
export async function getAgeGroupImpactBySexSeries(): Promise<AgeGroupImpactBySexPoint[]> {
